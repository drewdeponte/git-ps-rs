...
pub fn list(color: bool) -> Result<(), ListError> {
    ...
    if git::in_rebase(repo_gitdir_path) {
        ...
        if config.list.reverse_order {
            let done_todos_vec = git::in_rebase_done_todos(repo_gitdir_path).unwrap();
            println!(
                "Last commands done ({} commands done)",
                done_todos_vec.len()
            );
            for todo in done_todos_vec.iter() {
            ...
            for todo in done_todos_vec.iter().rev() {
                println!("{}", rebase_todo_command_to_row(todo, color));
            }
        }
        ...
        let cur_patch_stack_branch_name =
            str::replace(&cur_patch_stack_branch_ref, "refs/heads/", "");
        ...
        for patch in list_of_patches_iter {
            ...
            {
                Ok(id) => Some(id),
                Err(git::CommitDiffPatchIdError::GetDiffFailed(
                    git::CommitDiffError::MergeCommit,
                )) => None,
