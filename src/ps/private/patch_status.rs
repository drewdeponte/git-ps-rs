...
fn compute_pushed_to_remote_status(
    local_patch_has_changed: bool,
    remote_patch_has_changed: bool,
    patch_is_behind: Option<bool>,
) -> PatchStatus {
...
fn compute_request_reviewed_status(
    local_patch_has_changed: bool,
    remote_patch_has_changed: bool,
    patch_is_behind: Option<bool>,
) -> PatchStatus {
...
pub fn patch_status(
    patch_meta_data_option: Option<&state_management::Patch>,
    repo: &git2::Repository,
    commit_diff_patch_id: git2::Oid,
    patch_stack_base_oid: git2::Oid,
) -> Result<PatchStatus, PatchStatusError> {
    match patch_meta_data_option {
        None => Ok(PatchStatus::WithoutBranch),
        Some(patch_meta_data) => match &patch_meta_data.state {
            state_management::PatchState::BranchCreated(
                rr_branch_name,
                operation_diff_patch_id_string,
            ) => {
                let operation_diff_patch_id =
                    git2::Oid::from_str(operation_diff_patch_id_string)
                        .map_err(PatchStatusError::PatchIdFromPatchIdStringFailed)?;
                let local_patch_has_changed = commit_diff_patch_id != operation_diff_patch_id;

                match git::singular_commit_of_branch(
                    repo,
                    rr_branch_name,
                    git2::BranchType::Local,
                    patch_stack_base_oid,
                ) {
                    Ok(_) => Ok(compute_branched_status(local_patch_has_changed)),
                    Err(git::SingularCommitOfBranchError::BranchDoesntHaveExactlyOneCommit(
                        _,
                        _,
            ...
            state_management::PatchState::PushedToRemote(
                remote,
                rr_branch_name,
                operation_diff_patch_id_string,
                operation_remote_diff_patch_id_string,
            ) => {
                let operation_diff_patch_id =
                    git2::Oid::from_str(operation_diff_patch_id_string)
                        .map_err(PatchStatusError::PatchIdFromPatchIdStringFailed)?;
                let operation_remote_diff_patch_id =
                    git2::Oid::from_str(operation_remote_diff_patch_id_string)
                        .map_err(PatchStatusError::PatchIdFromPatchIdStringFailed)?;
                let local_patch_has_changed = commit_diff_patch_id != operation_diff_patch_id;

                match git::singular_commit_of_branch(
                    repo,
                    format!("{}/{}", remote, rr_branch_name).as_str(),
                    git2::BranchType::Remote,
                    patch_stack_base_oid,
                ) {
                    Ok(commit) => {
                        let is_behind = commit_is_behind(&commit, patch_stack_base_oid)
                            .map_err(PatchStatusError::GetCommitIsBehindFailed)?;

                        let remote_commit_diff_patch_id = git::commit_diff_patch_id(repo, &commit)
                            .map_err(PatchStatusError::GetCommitDiffPatchIdFailed)?;
                        let remote_patch_has_changed =
                            remote_commit_diff_patch_id != operation_remote_diff_patch_id;

                        Ok(compute_pushed_to_remote_status(
                            local_patch_has_changed,
                            remote_patch_has_changed,
                            Option::Some(is_behind),
                        ))
                    }
                    Err(git::SingularCommitOfBranchError::BranchDoesntHaveExactlyOneCommit(
                        _,
            ...
            state_management::PatchState::RequestedReview(
                remote,
                rr_branch_name,
                operation_diff_patch_id_string,
                operation_remote_diff_patch_id_string,
                ...
                match git::singular_commit_of_branch(
                    ...
                    Ok(commit) => {
                        ...
                        let remote_patch_has_changed =
                            remote_commit_diff_patch_id != operation_remote_diff_patch_id;
                        Ok(compute_request_reviewed_status(
                            local_patch_has_changed,
                            remote_patch_has_changed,
                            Option::Some(is_behind),
                        ))
