...
pub fn get_ssh_key_password(key_path: &str) -> std::result::Result<Option<String>, GetSshKeyPasswordError> {
    match keyring_entry(key_path) {
        Ok(entry) => {
            entry.get_password().map_err(|e| GetSshKeyPasswordError::Unknown(Box::new(e))).map(Some)
        },
...
pub fn set_ssh_key_password(key_path: &str, password: &str) -> std::result::Result<(), SetSshKeyPasswordError> {
    match keyring_entry(key_path) {
        Ok(entry) => {
            entry.set_password(password).map_err(|e| SetSshKeyPasswordError::Unknown(Box::new(e)))
        },
