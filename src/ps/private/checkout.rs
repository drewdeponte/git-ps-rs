...
pub fn checkout(repo: &git2::Repository, patch_index: usize) -> Result<(), CheckoutError> {
    ...
    let patches_vec =
        ps::get_patch_list(repo, &patch_stack).map_err(CheckoutError::GetPatchListFailed)?;
    let patch_oid = patches_vec
        .get(patch_index)
        .ok_or(CheckoutError::PatchIndexNotFound)?
        .oid;

    utils::execute("git", &["checkout", format!("{}", patch_oid).as_str()])
        .map_err(CheckoutError::FailedToCheckout)?;
